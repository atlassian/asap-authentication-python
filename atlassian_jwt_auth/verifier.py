...
class JWTAuthVerifier(object):
    ...
    def _decode_jwt(
        ...
        jwt_key: AllowedPublicKeys | PyJWK | str | bytes,
        ...
        if (
            not key_identifier.key_id.startswith("%s/" % claims["iss"])
            and key_identifier.key_id != claims["iss"]
        ):
        ...
        _iat = int(claims["iat"])
        if _exp - _iat > 3600:
            _msg = "Claims validity, '%s', exceeds the maximum 1 hour." % (_exp - _iat)
