...
from atlassian_jwt_auth.key import HTTPSMultiPublicKeyRetriever
from atlassian_jwt_auth.tests import utils


class BaseHTTPSMultiPublicKeyRetrieverTest(object):
    ...
    def test_https_public_key_retriever_does_not_support_http_url(self):
        ...
        with self.assertRaises(ValueError):
            retriever = HTTPSMultiPublicKeyRetriever(['http://example.com'])
    ...
    def test_https_public_key_retriever_does_not_support_none_url(self):
        ...
        """
        with self.assertRaises(ValueError):
            retriever = HTTPSMultiPublicKeyRetriever([None])

    def test_https_public_key_retriever_does_not_support_strings(self):
        """ tests that HTTPSPublicKeyRetriever does not support None
            base urls.
        """
        with self.assertRaises(ValueError):
            retriever = HTTPSMultiPublicKeyRetriever('https://foo.com')

    def test_https_public_key_retriever_supports_https_url(self):
        """ tests that HTTPSPublicKeyRetriever supports https://
            base urls.
        """
        retriever = HTTPSMultiPublicKeyRetriever(self.keystore_urls)
...
class HTTPSPublicKeyRetrieverRS256Test(BaseHTTPSMultiPublicKeyRetrieverTest,
                                       utils.RS256KeyTestMixin,
                                       unittest.TestCase):
    pass


class HTTPSPublicKeyRetrieverES256Test(BaseHTTPSMultiPublicKeyRetrieverTest,
